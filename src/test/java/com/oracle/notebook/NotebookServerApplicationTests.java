...
class NotebookServerApplicationTests {
	...
	public void testWithRequestInvalid() throws Exception {
		...
		this.mockMvc.perform(post("/execute")
					...
					.contentType(MediaType.APPLICATION_JSON))
					.andExpect(status().isOk())
					.andExpect(jsonPath("$.result", is("Invalid request format, use format like '%<interpreter-name><whitespace><code>'")));
	...
	public void testWithLanguageUnsupported() throws Exception {
		...
		this.mockMvc.perform(post("/execute")
					...
					.contentType(MediaType.APPLICATION_JSON))
					.andExpect(status().isOk())
					.andExpect(jsonPath("$.result", is("This language is unsupported, the only language suported is \" + Constants.LAGUAGE")));
	...
	public void testWithTooLongExecute() throws Exception {
		...
		this.mockMvc.perform(post("/execute").content(mapper.writeValueAsString(request))
					.contentType(MediaType.APPLICATION_JSON))
					.andExpect(status().isOk())
					.andExpect(jsonPath("$.result", is("Request taking too long to execute")));
